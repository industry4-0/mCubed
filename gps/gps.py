    ...
    with open(os.path.join('./logs/', filename), 'a+') as log:
        if (event == 'info'):
            ...
            logMessage = datetime.now().strftime(
                '%Y/%m/%d %H:%M:%S') + '\t' + ip + '\t' + client + '\t' + type + '\t' + data + '\n'
        elif (event == 'location'):
            # TSV format of: Timestamp, Client IP, Location DateTime, GPS/LBS, Validity, Nb Sat, Latitude, Longitude, Accuracy, Speed, Heading
            logMessage = datetime.now().strftime('%Y/%m/%d %H:%M:%S') + '\t' + ip + '\t' + client + '\t' + '\t'.join(
                list(str(x) for x in data.values())) + '\n'
...
def handle_client(client):
    ...
    while (True):
        ...
        try:
            ...
            if (len(packet) > 0):
                print(
                '[', addresses[client]['address'][0], ']', 'IN Hex :', packet.hex(), '(length in bytes =', len(packet),
                ')')
                keepAlive = read_incoming_packet(client, packet)
                LOGGER('info', 'server_log.txt', addresses[client]['address'][0], addresses[client]['imei'], 'IN',
                       packet.hex())
            ...
            # Close socket if recv() returns 0 bytes, i.e. connection has been closed
                ...
                print(
                '[', addresses[client]['address'][0], ']', 'DISCONNECTED: socket was closed for an unknown reason.')
                client.close()
                break
...
def read_incoming_packet(client, packet):
    ...
    if (protocol_name == 'login'):
        ...
        if (packet_list[0] == '06'):
            print(
            '[', addresses[client]['address'][0], ']', 'STATUS : Battery =', int(packet_list[2], base=16), '; Sw v. =',
            int(packet_list[3], base=16), '; Status upload interval =', int(packet_list[4], base=16))
        elif (packet_list[0] == '07'):
            print(
            '[', addresses[client]['address'][0], ']', 'STATUS : Battery =', int(packet_list[2], base=16), '; Sw v. =',
            int(packet_list[3], base=16), '; Status upload interval =', int(packet_list[4], base=16),
            '; Signal strength =', int(packet_list[5], base=16))
...
def answer_setup(query, uploadIntervalSeconds, binarySwitch, alarm1, alarm2, alarm3, dndTimeSwitch, dndTime1, dndTime2,
                 dndTime3, gpsTimeSwitch, gpsTimeStart, gpsTimeStop, phoneNumbers):
    """
    Synchronous setup
...
def answer_gps(client, query):
    ...
    if (dt != '000000000000'):
        dt = datetime.strftime(
            datetime.strptime(dt, '%y%m%d%H%M%S').replace(tzinfo=tz.tzutc()).astimezone(tz.tzlocal()), '%y%m%d%H%M%S')
    ...
    positions[client]['gps']['datetime'] = datetime.strptime(
        datetime.now().strftime('%y%m%d%H%M%S') if dt == '000000000000' else dt, '%y%m%d%H%M%S').strftime(
        '%Y/%m/%d %H:%M:%S')
    ...
    print(
    '[', addresses[client]['address'][0], ']', "POSITION/GPS : Valid =", position_is_valid, "; Nb Sat =", gps_nb_sat,
    "; Lat =", gps_latitude, "; Long =", gps_longitude, "; Speed =", gps_speed, "; Heading =", gps_heading)
    LOGGER('location', 'location_log.txt', addresses[client]['address'][0], addresses[client]['imei'], '',
           positions[client]['gps'])
...
def answer_wifi_lbs(client, query):
    ...
    if (n_wifi > 0):
        for i in range(n_wifi):
            current_wifi = {'macAddress': ':'.join(query[(8 + (7 * i)):(8 + (7 * (i + 1)) - 2 + 1)]),
                            # That +1 is because l[start:stop] returnes elements from start to stop-1...
                            'signalStrength': -int(query[(8 + (7 * (i + 1)) - 1)], base=16)}
            positions[client]['wifi'].append(current_wifi)

            # Print Wi-Fi hotspots into the logs
            print('[', addresses[client]['address'][0], ']', "POSITION/WIFI : BSSID =", current_wifi['macAddress'],
                  "; RSSI =", current_wifi['signalStrength'])
    ...
    if (n_gsm_cells > 0):
        for i in range(n_gsm_cells):
            current_gsm_cell = {'locationAreaCode': int(
                ''.join(query[(((8 + (7 * n_wifi)) + 4) + (5 * i)):(((8 + (7 * n_wifi)) + 4) + (5 * i) + 1 + 1)]),
                base=16),
                                'cellId': int(''.join(query[(((8 + (7 * n_wifi)) + 4) + (5 * i) + 1 + 1):(
                                            ((8 + (7 * n_wifi)) + 4) + (5 * i) + 2 + 1 + 1)]), base=16),
                                'signalStrength': -int(query[(((8 + (7 * n_wifi)) + 4) + (5 * i) + 2 + 1 + 1)],
                                                       base=16)}
            positions[client]['gsm-cells'].append(current_gsm_cell)

            # Print LBS data into logs as well
            print(
            '[', addresses[client]['address'][0], ']', "POSITION/LBS : LAC =", current_gsm_cell['locationAreaCode'],
            "; CellID =", current_gsm_cell['cellId'], "; MCISS =", current_gsm_cell['signalStrength'])

    # Build first stage of response with dt and send it to devices
    r_1 = make_content_response(hex_dict['start'] + hex_dict['start'], protocol, dt,
                                hex_dict['stop_1'] + hex_dict['stop_2'])
    print(
    '[', addresses[client]['address'][0], ']', 'OUT Hex :', r_1, '(length in bytes =', len(bytes.fromhex(r_1)), ')')
    ...
    if (list(decoded_position.keys())[0] == 'error'):
        ...
        positions[client]['gps']['latitude'] = '{0:{1}}'.format(decoded_position['location']['lat'],
                                                                '+' if decoded_position['location']['lat'] else '')
        positions[client]['gps']['longitude'] = '{0:{1}}'.format(decoded_position['location']['lng'],
                                                                 '+' if decoded_position['location']['lng'] else '')
    ...
    response = '2C'.join(
        [bytes(positions[client]['gps']['latitude'][0] + str(round(float(positions[client]['gps']['latitude'][1:]), 6)),
               'UTF-8').hex(),
         bytes(
             positions[client]['gps']['longitude'][0] + str(round(float(positions[client]['gps']['longitude'][1:]), 6)),
             'UTF-8').hex()])
    r_2 = make_content_response(hex_dict['start'] + hex_dict['start'], protocol, response,
                                hex_dict['stop_1'] + hex_dict['stop_2'])
    return (r_2)
...
def generic_response(protocol):
    ...
    """
    r = make_content_response(hex_dict['start'] + hex_dict['start'], protocol, None,
                              hex_dict['stop_1'] + hex_dict['stop_2'])
    return (r)
...
def make_content_response(start, protocol, content, stop):
    ...
    """
    return (start + format((len(bytes.fromhex(content)) if content else 0) + 1, '02X') + protocol + (
        content if content else '') + stop)
...
load_dotenv()
GMAPS_API_KEY = os.getenv('AIzaSyDhJiPUSmq1Y8RVW-etBsmmj9viBgzKd7c')
gmaps = googlemaps.Client(key="AIzaSyDhJiPUSmq1Y8RVW-etBsmmj9viBgzKd7c")
